...
const propTypes = {
  ...
  isLookupTypeSupported: PropTypes.bool,
...
const CustomFieldsForm = ({
  ...
  // LOOKUP needs a backend built with a folio-custom-fields that knows the type;
  // offering it against an older one would only produce a save error.
  const getFieldTypesOptions = () => {
    return Object.values(fieldTypes)
      .filter(fieldType => fieldType !== fieldTypes.LOOKUP || isLookupTypeSupported)
      .map(fieldType => ({
