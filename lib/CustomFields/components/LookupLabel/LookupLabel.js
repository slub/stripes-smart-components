...
const propTypes = {
  ...
  id: PropTypes.string,
  refEntityType: PropTypes.string.isRequired,
};
...
const LookupLabel = ({
  ...
  if (isUnknownSource) {
    return (
      <span data-test-lookup-unknown-source>
        <FormattedMessage
  ...
  if (hasLoadError) {
    return (
      <span data-test-lookup-load-error>
        <FormattedMessage
          id="stripes-smart-components.customFields.lookup.loadFailed"
          values={{ id }}
        />
      </span>
    );
