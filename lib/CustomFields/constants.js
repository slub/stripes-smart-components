...
import { LOOKUP_REF_ENTITY_TYPES } from './lookup';
...
export const fieldTypesLabelIds = {
  ...
  [fieldTypes.LOOKUP]: 'stripes-smart-components.customFields.fieldTypes.LOOKUP',
};
...
export const fieldTypesLabels = {
  ...
  [fieldTypes.LOOKUP]: <FormattedMessage id={fieldTypesLabelIds[fieldTypes.LOOKUP]} />,
};

export const NO_DEFAULT_OPTIONS_VALUE = 'no-default';
export const CUSTOM_FIELDS_SECTION_ID = 'default';

// Custom field sections (custom-fields interface 3.2): a custom field references
// its section by UUID in `sectionId`. Not to be confused with the `sectionId` prop
// of the record components, which filters by `displayInAccordion`.
export const CUSTOM_FIELD_SECTIONS_API = 'custom-field-sections';
export const CUSTOM_FIELD_SECTION_ID_PROPERTY = 'sectionId';
export const SECTION_IN_USE_ERROR_CODE = 'sectionInUse';
...
export const getDefaultFieldConfigs = ({ hasDisplayInAccordionField }) => ({
  ...
  [fieldTypes.LOOKUP]: {
    ...
    ...(hasDisplayInAccordionField && { displayInAccordion: CUSTOM_FIELDS_SECTION_ID }),
    lookupField: {
      refEntityType: LOOKUP_REF_ENTITY_TYPES.ORGANIZATION,
    },
  }
});
