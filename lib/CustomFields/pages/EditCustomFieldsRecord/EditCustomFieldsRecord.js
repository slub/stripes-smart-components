...
const EditCustomFieldsRecord = ({
  ...
  const allVisibleCustomFields = customFields?.filter(customField => customField.visible);
  ...
  const renderCustomField = customField => {
    ...
    } else if (customField.type === fieldTypes.LOOKUP) {
      return createCustomFieldRenderFunction(customField)({ refEntityType: customField.lookupField?.refEntityType });
    } else {
      return createCustomFieldRenderFunction(customField)();
    }
  };
  ...
  const renderChunkedCustomFields = (rows) => rows.map((row, i) => (
    <Row key={i}>
    ...
    return (
      <>
        {showDefaultAccordion && (
          ...
          >
            {customFieldsLoaded && renderChunkedCustomFields(formattedCustomFields)}
          </Accordion>
        )}
