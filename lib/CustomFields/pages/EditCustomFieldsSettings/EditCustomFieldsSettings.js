...
import {
  selectCustomFieldsInterfaceVersion,
  selectModuleId,
  selectOkapiData,
} from '../../selectors';
import { isLookupTypeSupported } from '../../lookup';
...
const propTypes = {
  ...
  customFieldsInterfaceVersion: PropTypes.string,
...
const EditCustomFieldsSettings = ({
  ...
  } = useLookupFieldsStatsFetch(okapi, backendModuleId, latestCustomFields, entityType);
  ...
  const formatCustomFieldsForBackend = (customFieldsFormData, systemCustomFieldsData) => {
    if (Array.isArray(customFieldsFormData.customFields)) {
      const formattedFormFields = customFieldsFormData.customFields.map(({ values: { hidden, ...customFieldData }, id }) => {
        ...
        // an empty section select means the default accordion: the backend expects no `sectionId` at all
        if (!customFieldDataWithSortedOptions[CUSTOM_FIELD_SECTION_ID_PROPERTY]) {
          delete customFieldDataWithSortedOptions[CUSTOM_FIELD_SECTION_ID_PROPERTY];
        }
          ...
          permissions.canEdit
            ? (
              <CustomFieldsForm
                ...
                isLookupTypeSupported={isLookupTypeSupported(customFieldsInterfaceVersion)}
                usedLookupFieldIds={usedLookupFieldIds}
                lookupStatsLoaded={lookupStatsLoaded}
                ...
                customFieldSectionOptions={hasCustomFieldSections
...
export default connect(
  (state, ownProps) => {
    const backendModuleId = selectModuleId(state, ownProps.backendModuleName);

    return {
      backendModuleId,
      customFieldsInterfaceVersion: selectCustomFieldsInterfaceVersion(state, backendModuleId),
