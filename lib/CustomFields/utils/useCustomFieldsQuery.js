...
import { applyLookupOverlay } from '../poc/lookupFieldOverlay';
...
const useCustomFieldsQuery = ({
  ...
  sectionId,
  isVisible,
  // PoC only. Must stay off in settings: a promoted field would be written back to
  // the backend as LOOKUP and rejected. See poc/lookupFieldOverlay.js.
  pocLookupOverlay = false,
}) => {
  const [namespace] = useNamespace({ key: 'customFields' });
  const stripes = useStripes();
  const { store: { getState } } = stripes;
  const state = getState();
  const ky = useOkapiKy();
  const callout = useCallout();
  const intl = useIntl();

  const lookupOverlay = stripes.config?.lookupPocFields;
  ...
  const customFields = useMemo(() => {
    ...
    const source = pocLookupOverlay ? applyLookupOverlay(data, lookupOverlay) : data;

    let fields = source.filter(customField => {
    ...
    return fields;
  }, [data, entityType, sectionId, isVisible, pocLookupOverlay, lookupOverlay, callout, intl]);
