...
const useLookupFieldsStatsFetch = (okapi, backendModuleId, customFields, entityType) => {
  ...
  useEffect(() => {
    ...
    const fetchFieldsUsageStatistics = async () => {
      const promises = customFields
        .filter(customField => customField.type === fieldTypes.LOOKUP && isSaved(customField))
        .map(customField => getFieldUsageStatistics(customField.id));
