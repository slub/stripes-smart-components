 ...
 * instead of being thrown. A source this frontend has no adapter for, and a request
 * that failed for any other reason, are reported separately as well: in neither case
 * is the reference known to be broken.
 */
const useLookupRecord = ({ refEntityType, id }) => {
  ...
  return {
    ...
    isLoadingRecord: Boolean(adapter && id) && isLoading,
    isUnknownSource: !adapter,
    isForbidden,
    isNotFound,
    hasLoadError: Boolean(error) && !isForbidden && !isNotFound,
